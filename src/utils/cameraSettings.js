...
export const WHITE_BALANCE_OPTIONS = [
  { id: 'auto', label: 'AUTO', rMul: 1, bMul: 1, previewFilter: '', kelvin: null },
  {
    id: 'daylight',
    label: 'DAYLIGHT',
    rMul: 1.03,
 ...
 * crop) when there's no hardware zoom capability at all — 0.5x is
 * excluded from that fallback too, since it only ever comes from real
 * hardware.
 */
export const getAvailableZoomLevels = (capabilities) => {
  const withoutUltraWide = ZOOM_LEVELS.filter((l) => l !== ULTRA_WIDE_ZOOM);
  if (!capabilities) return withoutUltraWide;
  const { min, max } = capabilities;
  const levels = withoutUltraWide.filter((l) => l >= min - 0.001 && l <= max + 0.001);
  if (min <= ULTRA_WIDE_ZOOM + 0.001) levels.unshift(ULTRA_WIDE_ZOOM);
