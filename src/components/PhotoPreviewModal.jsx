    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-body-black/95 px-6 backdrop-blur-sm">
            ...
            <div className="w-full max-w-sm rounded-sm bg-cream p-2 pb-6 shadow-2xl">
                <div
                    className="w-full overflow-hidden rounded-[1px] bg-black"
                    style={{ aspectRatio: meta?.ratioValue ?? 3 / 4 }}
                >
                    <img
                        src={photo}
                        alt="Developed film photo"
                ...
                {meta && (
                    <p className="mt-1 text-center font-mono text-[10px] tracking-widest text-body-black/45">
                        F/{meta.aperture} · {meta.ratioLabel}
                        {meta.exposureEv
                            ? ` · ${meta.exposureEv > 0 ? "+" : ""}${meta.exposureEv}EV`
                            : ""}
                        {meta.whiteBalanceLabel &&
                        meta.whiteBalanceLabel !== "AUTO"
                            ? ` · ${meta.whiteBalanceLabel}`
                            : ""}
                    </p>
                )}
