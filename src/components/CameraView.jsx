...
import { FILM_STOCKS, ISO_VALUES, DEFAULT_STOCK_ID, DEFAULT_ISO, DEFAULT_FLASH, getStock } from '../utils/filmStocks';
...
const FlashIcon = ({ mode }) => {
  ...
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill={mode === 'on' ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.8">
      <path d="M13 2 4 14h6l-1 8 9-12h-6l1-8Z" strokeLinejoin="round" />
    </svg>
  );
};
...
const GridIcon = ({ active }) => (
  <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={active ? 2.2 : 1.8}>
...
export default function CameraView({ onCapture, framesLeft, disabled }) {
  const { videoRef, facingMode, isReady, error, switchCamera, retry, torchSupported, setTorch } = useCamera();
  ...
  const cycleFlash = useCallback(() => {
    ...
    setFlashMode((current) => order[(order.indexOf(current) + 1) % order.length]);
  }, []);
  ...
  const runCapture = useCallback(async () => {
    ...
    const canUseTorch = willFlash && torchSupported && facingMode === 'environment';
    ...
    try {
      ...
      onCapture({ dataUrl, meta: { filterLabel: stock.label, iso, flashFired: willFlash } });
    } catch (err) {
  ...
  }, [videoRef, flashMode, torchSupported, facingMode, setTorch, stockId, iso, stock.label, onCapture]);
  ...
  }, [countdown, disabled, isCapturing, isReady, error, timerSeconds, runCapture]);
    ...
    <div className="relative flex h-full w-full flex-col bg-body-black">
      ...
      <div className="leatherette safe-top flex items-center justify-between border-b border-panel-brown-light bg-panel-brown px-4 py-2 text-lcd-amber">
        <div className="flex items-center gap-2 text-xs tracking-widest">
          <span className={`h-1.5 w-1.5 rounded-full bg-film-red ${isReady && !error ? 'blink-dot' : ''}`} />
        ...
        <div className="flex items-center gap-3 font-mono text-[11px] tracking-wide">
          ...
          <span className="tabular-nums">{String(framesLeft).padStart(2, '0')} EXP</span>
        </div>
      </div>
      ...
      <div className="relative flex flex-1 items-center justify-center overflow-hidden bg-black">
        <div className="relative aspect-[3/4] h-full max-h-full w-full overflow-hidden bg-black sm:h-auto sm:max-h-[calc(100dvh-13rem)] sm:w-auto">
          ...
          {/* capture flash */}
          {flashVisual && <div className="pointer-events-none absolute inset-0 bg-cream shutter-flash" />}
            ...
            <div className="pointer-events-none absolute left-1/2 top-1/2 w-2/3 -translate-x-1/2 -translate-y-1/2">
              <div
                ...
                style={{
                  ...
                  backgroundColor: isLevelFlat ? '#7ddc8c' : 'rgba(237,227,211,0.6)',
                }}
              />
              <div
                className="mx-auto -mt-[3px] h-1.5 w-1.5 rounded-full transition-colors"
                style={{ backgroundColor: isLevelFlat ? '#7ddc8c' : 'rgba(237,227,211,0.8)' }}
              />
            </div>
          )}
          ...
          {countdown !== null && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <span key={countdown} className="countdown-pulse font-display text-8xl font-semibold text-cream drop-shadow-[0_0_18px_rgba(0,0,0,0.6)]">
                {countdown}
              </span>
            </div>
          )}
          ...
          {!showFilters && (
            <div className="absolute right-3 top-3 flex flex-col items-end gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-full bg-body-black/60 px-2.5 py-1 font-mono text-[10px] tracking-widest text-cream backdrop-blur-sm"
              >
                <span className="h-2.5 w-2.5 rounded-full border border-cream/50" style={{ backgroundColor: stock.swatch }} />
                FILM
              </button>
            </div>
          )}
          ...
          {error && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-body-black/90 px-6 text-center">
              <p className="font-mono text-sm text-cream/90">{error}</p>
              <button
                onClick={retry}
          ...
          {!isReady && !error && (
            <div className="absolute inset-0 flex items-center justify-center bg-body-black/60">
              <span className="font-mono text-xs tracking-widest text-cream/70">STARTING CAMERA&hellip;</span>
            </div>
          )}
        </div>
      </div>
      ...
      {showFilters && (
        <div className="leatherette border-t border-panel-brown-light bg-panel-brown px-4 py-3">
          <div className="mb-2 flex items-center justify-between">
            <span className="font-mono text-[10px] tracking-widest text-metal-light">CHOOSE FILM STOCK</span>
            <button
              ...
              >
                <span
                  className={`h-11 w-11 rounded-full border-2 transition ${
                    s.id === stockId ? 'border-film-orange scale-110' : 'border-metal/60'
                  }`}
                  style={{ backgroundColor: s.swatch }}
                />
                <span className={`font-mono text-[9px] tracking-wide ${s.id === stockId ? 'text-lcd-amber' : 'text-metal-light'}`}>
                  {s.label}
                </span>
              </button>
            ))}
      ...
      <div className="leatherette safe-bottom flex items-center justify-between border-t border-panel-brown-light bg-panel-brown px-8 py-5">
        ...
        >
          <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.8">
            ...
            <path d="M15.5 5.5 17 4l1.5 1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
        ...
        <div className="flex h-12 w-12 flex-col items-center justify-center rounded-full border border-metal bg-panel-brown-light font-mono text-lcd-amber">
          <span className="text-[9px] leading-none text-metal-light">FRAME</span>
          <span className="text-sm font-semibold leading-none">{String(framesLeft).padStart(2, '0')}</span>
        </div>
      </div>
    </div>
  );
